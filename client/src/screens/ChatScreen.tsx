...
const ChatScreen = () => {
  ...
  const targetId = route.params?.partnerId || route.params?.userId;
  const partner = nearbyUsers.find((u) => u.id === targetId);

  const [messageInput, setMessageInput] = useState('');
  const [messages, setMessages] = useState(chatHistory[targetId || ''] || []);
    ...
    return (
      <View className="flex-1 items-center justify-center bg-background">
        <View className="items-center">
          <Text className="mb-2 font-technical uppercase text-muted-foreground">
            User_Not_Found
          </Text>
      ...
      >
        <View className="mb-3 flex-row items-center justify-between">
          <View className="flex-row items-center gap-3">
            ...
            <View>
              <Text className="font-body font-medium text-foreground">{partner.name}</Text>
          ...
          <TouchableOpacity className="flex-row items-center rounded-sm bg-accent px-3 py-1 transition-opacity hover:opacity-90">
            <Ionicons
              ...
              style={{ marginRight: 4 }}
            />
            <Text className="font-technical text-sm uppercase tracking-wider text-accent-foreground">
              Complete
            </Text>
          </TouchableOpacity>
        </View>
        ...
        {/* Context Bar */}
        <View className="bg-primary/10 rounded-sm border-l-4 border-l-primary px-3 py-2">
          <Text className="mb-1 font-technical text-xs uppercase tracking-wider text-primary">
            Context_Bar
          </Text>
      ...
      <View className="relative flex-1 bg-muted">
                ...
                <View key={message.id} className="group mb-4">
                  <View className={`flex-row ${isSent ? 'justify-end' : 'justify-start'}`}>
                    <View className="max-w-[75%]">
                      ...
                      <Text
                        className={`mt-1 px-1 font-technical text-xs text-muted-foreground ${isSent ? 'text-right' : 'text-left'}`}
                      >
                  ...
                  {isSent && (
                    ...
                    >
                      ...
                      >
                        ...
                        <Text className="ml-1 font-technical text-xs uppercase text-[#4F46E5]">
                          Edit
                        </Text>
                      </TouchableOpacity>
                      ...
                      >
                        ...
                        <Text className="ml-1 font-technical text-xs uppercase text-[#EF4444]">
                          Delete
                        </Text>
                      </TouchableOpacity>
                    </View>
        ...
        {showJumpToBottom && (
          <TouchableOpacity
            ...
            className="absolute bottom-4 h-12 w-12 items-center justify-center rounded-full border border-border bg-card shadow-lg ring-4"
            style={{ elevation: 5 }}
          >
      ...
      >
        ...
        <View className="flex-row gap-2">
          <TextInput
            ...
            className="flex-1 rounded-sm border-2 border-border bg-muted px-4 py-3 font-body text-foreground transition-colors focus:border-primary focus:outline-none"
          />
          <TouchableOpacity
            onPress={handleSendMessage}
            className="items-center justify-center rounded-sm bg-primary px-6 transition-opacity hover:opacity-90"
          >
